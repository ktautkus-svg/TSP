    ...
    <ScrollView contentContainerStyle={styles.page}>
      <View style={styles.topbar}>
        <View style={styles.heading}>
          <Text style={styles.eyebrow}>TSP · KOKYBĖS KONTROLĖ</Text>
          <Text style={styles.pageTitle}>Maršrutai gyvai</Text>
          <Text style={styles.subtitle}>Tik stebėjimas · duomenys automatiškai atnaujinami kas 15 sekundžių</Text>
        </View>
        <View style={styles.livePanel}>
          <View style={[styles.liveDot, !online && styles.liveDotOffline]} />
          <View><Text style={styles.liveLabel}>{online ? 'GYVAS RYŠYS' : 'RYŠIO NĖRA'}</Text><Text style={styles.refreshTime}>Atnaujinta {formatClock(lastRefreshedAt)}</Text></View>
      ...
      <View style={styles.metrics}>
        <SummaryMetric label="KELYJE" value={active.length} tone="info" styles={styles} />
        <SummaryMetric label="LAUKIA STARTO" value={waiting.length} tone="warning" styles={styles} />
        <SummaryMetric label="BAIGTA ŠIANDIEN" value={completed.length} tone="success" styles={styles} />
        <SummaryMetric label="PRISTATYTA TAŠKŲ" value={deliveredToday} tone="neutral" styles={styles} />
      </View>
      ...
      {active.length > 0 ? <RouteSection title="Dabar vykdomi" count={active.length} routes={active} desktop={desktop} styles={styles} /> : null}
      {waiting.length > 0 ? <RouteSection title="Laukia pradžios" count={waiting.length} routes={waiting} desktop={desktop} styles={styles} /> : null}
      {completed.length > 0 ? <RouteSection title="Šiandien užbaigti" count={completed.length} routes={completed} desktop={desktop} styles={styles} /> : null}
    </ScrollView>
  </SafeAreaView>;
}

function RouteSection({ title, count, routes, desktop, styles }: { title: string; count: number; routes: QualityRouteMonitor[]; desktop: boolean; styles: ReturnType<typeof createStyles> }) {
  return <View style={styles.section}>
    <View style={styles.sectionHeading}><Text style={styles.sectionTitle}>{title}</Text><Text style={styles.count}>{count}</Text></View>
    <View style={[styles.routeGrid, desktop && styles.routeGridDesktop]}>{routes.map((route) => <RouteCard key={route.id} route={route} desktop={desktop} styles={styles} />)}</View>
  </View>;
}

function RouteCard({ route, desktop, styles }: { route: QualityRouteMonitor; desktop: boolean; styles: ReturnType<typeof createStyles> }) {
  ...
  return <View style={[styles.routeCard, desktop && styles.routeCardDesktop, completed && styles.routeCardCompleted]} testID={`quality-route-${route.id}`}>
    ...
    <View style={styles.routeMeta}><Text style={styles.routeNumber}>{route.routeNumbers.join(', ') || 'R numeris nenurodytas'}</Text><Text style={styles.muted}>{formatDate(route.date)}</Text></View>
    ...
    {route.nextStop ? <View style={styles.nextStop}>
      ...
      <View style={styles.flex}><Text style={styles.nextRecipient}>{route.nextStop.recipient}</Text><Text numberOfLines={2} style={styles.nextAddress}>{route.nextStop.address}</Text><Text style={styles.nextMeta}>{route.nextStop.routeNumber ?? 'Be R numerio'} · {formatWeightKg(route.remainingWeightKg)} kg likę</Text></View>
...
function SummaryMetric({ label, value, tone, styles }: { label: string; value: number; tone: 'info' | 'warning' | 'success' | 'neutral'; styles: ReturnType<typeof createStyles> }) {
  return <View style={styles.metric}><View style={[styles.metricAccent, styles[`metricAccent_${tone}`]]} /><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricLabel}>{label}</Text></View>;
}
...
const createStyles = (colors: ColorPalette) => StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: colors.background }, page: { width: '100%', maxWidth: 1440, alignSelf: 'center', padding: spacing.xl, paddingBottom: 80, gap: spacing.xl },
  ...
  eyebrow: { ...type.label, color: colors.info }, pageTitle: { ...type.pageTitle, color: colors.text, fontSize: 34, lineHeight: 40 }, subtitle: { ...type.body, color: colors.textMuted },
  ...
  metrics: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md }, metric: { flexGrow: 1, minWidth: 185, minHeight: 106, padding: spacing.lg, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, overflow: 'hidden' },
  ...
  metricValue: { fontSize: 32, lineHeight: 38, fontFamily: fonts.heading, color: colors.text }, metricLabel: { ...type.label, color: colors.textMuted, marginTop: spacing.xs }, warning: { ...type.bodyStrong, padding: spacing.md, borderRadius: radius.md, backgroundColor: colors.warningSoft, color: colors.warning },
  ...
  routeGrid: { gap: spacing.md }, routeGridDesktop: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'stretch' }, routeCard: { padding: spacing.lg, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: colors.surface, gap: spacing.md }, routeCardDesktop: { width: '48.9%', flexGrow: 1, maxWidth: '50%' }, routeCardCompleted: { borderColor: colors.border },
