...
export default function RouteOverviewScreen() {
  ...
  return <View style={styles.screen}>
    <Stack.Screen options={{
      ...
      headerLeft: () => <Pressable onPress={() => router.replace('/history' as Href)} style={styles.headerAction}><Text style={styles.headerText}>← Maršrutai</Text></Pressable>,
    }} />
    <FoundationScreen showFoundationNotice={false} title="Maršruto informacija" description="Prieš pradėdami peržiūrėkite visą eigą ir taškų eiliškumą.">
      ...
      {route ? <>
        ...
        <View style={styles.actions}>
          <Pressable style={styles.primaryButton} onPress={begin}><Text style={styles.primaryText}>{route.status === 'in_progress' ? 'Tęsti maršrutą' : 'Pradėti maršrutą'}</Text></Pressable>
...
const createStyles = (colors: ColorPalette) => StyleSheet.create({
  ...
  headerAction: { minWidth: 110, minHeight: 44, justifyContent: 'center' }, headerText: { ...type.button, color: colors.textInverse },
